...
router.post("/candidate/create",jwtAuthMiddleware, isAdmin, async (req, res) => {
  try {
...
router.put("/candidate/update/:id", jwtAuthMiddleware, isAdmin, async (req, res) => {
    try {
      ...
      const updatedCandidateData = await Candidate.findByIdAndUpdate(candidateId, updateCandidate, {
...
// Delete route for permanently removing Candidate data from the database by ID
router.delete("/candidate/delete/:id", jwtAuthMiddleware, isAdmin,async (req, res) => {
  try {
